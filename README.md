...
**Instead of:** "A cat"
**Try:** "A fluffy orange cat sleeping on a windowsill with sunbeams shining through"

**Instead of:** "A landscape"
**Try:** "Mountain landscape at sunset with a river flowing through the valley, photorealistic style"
...
**"Server not starting"**
- Make sure port 8000 isn't already being used by another program
- Try running: `python main.py --port 8001`
...
- Your HuggingFace token might be invalid
- Go back to HuggingFace and make sure you copied the token correctly

**Slow image generation**
- The FLUX.1 model is generally faster than SD-XL
- Complex descriptions take longer to process
...
- `POST /api/generate` - Generate images (send JSON with "prompt" and "model_key")
- `GET /api/status` - Check if the system is working
- `GET /api/models` - See which models are available
